...
import discord
from lib.bot.firebaseauth import getUser, getUserMail, getUIDByQuestionID, deleteQuestion
from lib.bot.directoryOperations import scanTheDir, scanDir, deleteDir
from lib.bot.log import Log
import time

PREFIX = "+"
OWNER_IDS = [183827057300471808]
COGS = [path.split(os.sep)[-1][:-3] for path in glob(r"C:\Users\minh303\Documents\School Folder\Python\api\Chegg-Discord-BOT-main\Chegg-Discord-BOT-INIT-2\lib\cogs\fun.py")]
...
class Bot(BotBase):
    def __init__(self):
        ...
        self.log = Log(r"C:\Users\minh303\Documents\School Folder\Python\api\Chegg-Discord-BOT-main\Chegg-Discord-BOT-INIT-2", "chegg.log")
    ...
    def run(self, version):
        ...
        with open(r"C:\Users\minh303\Documents\School Folder\Python\api\Chegg-Discord-BOT-main\Chegg-Discord-BOT-INIT-2\lib\bot\token.0", "r", encoding="utf-8") as tf:
            self.TOKEN = tf.read()
    ...
    async def on_ready(self):
        ...
        if not self.ready:
            #799828505910575116
            self.channel = self.get_channel(818181844742176791)
            self.ready = True
            self.cogs_ready = Ready()
            self.guild = self.get_guild(818181844742176791)
            self.stdout = self.get_channel(818181844742176791)
            ...
            downloadsPath = r"C:\Users\minh303\Documents\School Folder\Python\api\Chegg-Discord-BOT-main\Chegg-Discord-BOT-INIT-2\Downloads"
            while True:
                time.sleep(1)
                ...
                if scannedQuestionId != None:
                    ...
                    if askerUserId == None:
                        deleteDir(r"{0}\{1}".format(downloadsPath, scannedQuestionId))

                    else:
                        user = await self.fetch_user(int(askerUserId))
                        print(user)
                        files = scanTheDir(r"{0}\{1}".format(downloadsPath, scannedQuestionId))
